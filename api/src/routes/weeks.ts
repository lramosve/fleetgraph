...
router.get('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Uses CTEs to pre-aggregate issue counts and retro data instead of 8 correlated subqueries
    const result = await pool.query(
      `WITH issue_stats AS (
         ...
         FROM document_associations ida
         JOIN documents i ON i.id = ida.document_id AND i.document_type = 'issue'
         WHERE ida.relationship_type = 'sprint'
         GROUP BY ida.related_id
       ),
       plan_exists AS (
         SELECT parent_id, true as has_plan
         FROM documents
       ...
       retro_info AS (
         ...
         WHERE rda.relationship_type = 'sprint'
       ...
       SELECT d.id, d.title, d.properties, prog_da.related_id as program_id,
              ...
              COALESCE(pe.has_plan, false) as has_plan,
       ...
       LEFT JOIN plan_exists pe ON pe.parent_id = d.id
