...
import fleetgraphRoutes from './fleetgraph/routes/fleetgraph.js';
...
const isBenchmarkMode = process.env.BENCHMARK_NO_RATE_LIMIT === '1';
...
const loginLimiter = rateLimit({
  ...
  max: isBenchmarkMode ? 999999 : isTestEnv ? 1000 : 5, // Very high for benchmarks
...
const apiLimiter = rateLimit({
  ...
  max: isBenchmarkMode ? 999999 : isTestEnv ? 10000 : isDevEnv ? 1000 : 100, // Very high for benchmarks
  standardHeaders: true,
  legacyHeaders: false,
...
export function createApp(corsOrigin: string = 'http://localhost:5173'): express.Express {
  ...
  // FleetGraph agent routes (CSRF protected for POST endpoints)
